...
def main():
    ...
    folders = [f for f in glob.glob(args.in_path+"*.jpg" , recursive=True)]
    i=0
    for f in folders:
        i=i+1
        image = Image.open(f).convert('RGB')
        target = Image.open(f).convert('L')
        ...
        save_image(grid_image, args.out_path+str(i)+".jpg")
